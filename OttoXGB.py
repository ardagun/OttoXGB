...
#X_train, X_val, y_train, y_test = train_test_split( X_scaled, y_encoded, random_state=0,stratify=y)
...
#predictions_RF=clf_xgb.predict(test_X)
#predictions_RF
#pred_proba=clf_xgb.predict_proba(test_X)

#idd=np.asarray(prediction_df['id'])
#a=pd.Series(idd)
#array = pd.DataFrame(pred_proba,columns=["Class_1","Class_2","Class_3","Class_4","Class_5","Class_6","Class_7","Class_8","Class_9"])
...
#final.to_csv("XGB.csv",index=None)